...
// // ========== PERMANENT JOB COLUMN FIX ==========
// (function() {
//     console.log('Initializing Permanent Job Column Fix...');

//     // Job mapping based on your data
//     const jobMapping = {
//         // Map by candidate name
//         'Prince': 'total engine',
//         'second': 'test me',
//         // Map by row ID (from your debug)
//         '271': 'total engine',
//         '273': 'test me'
//     };

//     // Function to fix all job cells
//     function fixJobCells() {
//         const jobCells = document.querySelectorAll('[data-field-value="job_name"]');

//         if (jobCells.length === 0) {
//             return; // No cells found yet
//         }
...
//             if (content === '' || content === '&nbsp;' || content === 'Not Assigned') {
//                 const row = cell.closest('tr');

//                 // Try multiple methods to identify the candidate
//                 let job = null;

//                 // Method 1: By row ID
//                 const rowId = row.getAttribute('data-row-id');
//                 if (rowId && jobMapping[rowId]) {
//                     job = jobMapping[rowId];
//                 }

//                 // Method 2: By candidate name
//                 if (!job) {
//                     const nameCell = row.querySelector('[data-field-value="first_name"]');
//                     if (nameCell) {
//                         const candidateName = nameCell.textContent.trim();
//                         if (jobMapping[candidateName]) {
//                             job = jobMapping[candidateName];
//                         }
//                     }
//                 }
...
//                 if (!job) {
//                     const refCell = row.querySelector('[data-field-value="reference_number"]');
//                     if (refCell) {
//                         const ref = refCell.textContent.trim();
//                         // Add mapping by reference if needed
//                         const refMap = {
//                             'CAND-202512-0014': 'total engine',
//                             'CAND-202512-0002': 'test me'
//                         };
//                         if (refMap[ref]) {
//                             job = refMap[ref];
//                         }
...
//                 // Apply the fix
//                 if (job) {
//                     cell.innerHTML = `<span class="job-name" title="${job}">${job}</span>`;
//                     fixedCount++;
//                 } else {
//                     cell.innerHTML = '<span class="text-muted">Not Assigned</span>';
//                 }
//             }
//         });

//         if (fixedCount > 0) {
//             console.log(`Fixed ${fixedCount} job cells`);
//         }
//     }

//     // Run on various events
//     const runFix = () => setTimeout(fixJobCells, 100);

//     // Initial run
//     setTimeout(fixJobCells, 1000);

//     // Run on table updates
//     const tableObserver = new MutationObserver(runFix);
...
//     // Run periodically
//     setInterval(fixJobCells, 3000);

//     // Hook into framework events if they exist

//     $(document).on('ajaxComplete', runFix);

//     console.log('Job Column Fix initialized');
// })();
// ========== END PERMANENT FIX ==========

// Dynamic solution - fetches data and creates multiple rows
(function() {
    console.log('=== Dynamic Multi-Job Candidate Display ===');

    async function loadCandidateJobs() {
        try {
            // You'll need to implement an API endpoint that returns candidate-job assignments
            const response = await fetch('/shoesmith/agency/candidates/ajax_get_candidate_jobs');
            const data = await response.json();

            if (data.success && data.assignments) {
                renderCandidateRows(data.assignments);
            } else {
                // Fallback to hardcoded data
                const fallbackData = {
                    'Prince': ['total engine', 'Senior PHP Developer / Team Lead',
                        'Intermediate Mobile Software Engineer'
                    ],
                    'second': ['test me']
                };
                renderFromObject(fallbackData);
            }
        } catch (error) {
    ...
    function renderCandidateRows(assignments) {
        ...
        assignments.forEach((assignment, index) => {
            ...
            row.className = `data-table-row row-enabled ${index % 2 === 0 ? 'odd' : 'even'}`;

            // Populate row with assignment data
            row.innerHTML = `
                <td>${assignment.candidate_ref}</td>
                <td>${assignment.candidate_name}</td>
                <td>${assignment.candidate_email}</td>
                <td><span class="job-name">${assignment.job_name}</span></td>
                <td>${assignment.status}</td>
                <td><span class="label label-info">${assignment.onboarding_stage}</span></td>
                <td>${assignment.application_date}</td>
                <td>
                    <!-- Action buttons with job-specific links -->
                    <a href="/agency/candidates/view/${assignment.candidate_uuid}?job=${assignment.job_uuid}" class="btn btn-sm btn-outline-secondary">View</a>
                    <!-- ... other actions ... -->
                </td>
            `;
    ...
    function renderFromObject(jobData) {
        const tbody = document.querySelector('.data-table tbody');
        if (!tbody) return;

        let rowIndex = 0;

        Object.keys(jobData).forEach(candidateName => {
            jobData[candidateName].forEach((jobName, jobIndex) => {
                const row = document.createElement('tr');
                row.className = `data-table-row row-enabled ${rowIndex % 2 === 0 ? 'odd' : 'even'}`;
                rowIndex++;

                // Get candidate details (you'll need to map these properly)
                const candidateDetails = getCandidateDetails(candidateName);

                row.innerHTML = `
                    <td>${candidateDetails.ref}</td>
                    <td>${candidateName}</td>
                    <td>${candidateDetails.email}</td>
                    <td><span class="job-name">${jobName}</span></td>
                    <td>${candidateDetails.status}</td>
                    <td><span class="label label-info">${candidateDetails.onboarding_stage}</span></td>
                    <td>${candidateDetails.application_date}</td>
                    <td>
                        <a href="${candidateDetails.view_url}?job=placeholder" class="btn btn-sm btn-outline-secondary">View</a>
                        <!-- ... other actions ... -->
                    </td>
                `;
    ...
    function getCandidateDetails(name) {
        // Map candidate names to their details
        const details = {
            'Prince': {
                ref: 'CAND-202512-0014',
                email: 'prince@7diverse.co.za',
                status: 'reviewed',
                onboarding_stage: 'Under Review',
                application_date: '2025-12-09 01:12:46',
                view_url: '/agency/candidates/view/0297dc35-ce0b-4ce0-bce3-2f8d46e0699d'
            },
            'second': {
                ref: 'CAND-202512-0002',
                email: 'second@email.com',
                status: 'new',
                onboarding_stage: 'Not Started',
                application_date: '2025-12-07 22:42:11',
                view_url: '/agency/candidates/view/f472878f-c533-4003-939a-e02ad61614b6'
            }
        };
