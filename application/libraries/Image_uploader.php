...
class Image_uploader {
    ...
    // ✅ ADDED: Safe upload directories
    private $allowed_upload_dirs = array();
    ...
    public function __construct($params=array()) {
        ...
        // ✅ ADDED: Define allowed upload directories
        $this->allowed_upload_dirs = array(
            realpath(FCPATH . 'resources/uploads/images/') ?: '',
            realpath(FCPATH . 'resources/uploads/thumbnails/') ?: '',
            realpath(FCPATH . 'uploads/') ?: '',
            realpath(FCPATH . 'uploads/images/') ?: ''
        );
        
        // Filter out empty paths
        $this->allowed_upload_dirs = array_filter($this->allowed_upload_dirs);
    }
    ...
    private function validateImagePath($path) {
        ...
        if ($realPath === false) {
            error_log('SECURITY: Invalid path - ' . $path);
            throw new Exception('Invalid image path');
        }
        
        // Check if path is within allowed directories
        $isAllowed = false;
        foreach ($this->allowed_upload_dirs as $allowedDir) {
        ...
        if (!$isAllowed) {
            error_log('SECURITY: Path traversal attempt - ' . $path . ' | Real: ' . $realPath);
        ...
        if (strpos($filename, '..') !== false || 
            ...
            strpos($filename, '..\\') !== false) {
            error_log('SECURITY: Path traversal in filename - ' . $filename);
    ...
    public function createThumbnailImage($thumb_image_name, $image, $width, $height=false) {
        // ✅ ADDED: Validate path BEFORE processing
        $this->validateImagePath($thumb_image_name);
        ...
        // ✅ FIXED: Secure permissions (NOT 0777!)
            ...
            foreach ($_FILES['imagefile']['tmp_name'] as $key => $tmp_name) {
                ...
                if ($_FILES['imagefile']['error'][$key] == 0) {
                    ...
                    if (empty($imageData[$i]['error'])) {
                        ...
                        // ✅ ADDED: Validate upload path
                        $this->validateImagePath($upload_file_location);
                        ...
                        if (move_uploaded_file($imageData[$i]['temp_name'], $upload_file_location)) {
                            ...
                            if (!empty($this->image_sizes)){
                                foreach ($this->image_sizes as $folder => $size) {
                                    ...
                                    // ✅ ADDED: Validate thumbnail path
                                    $this->validateImagePath($thumb_file_name);
        ...
        if (isset($_POST["upload_thumbnail"])) {
            ...
            foreach ($image_thumbnails as $key => $value) {
                if (!is_dir($upload_path . $key)) {
                    ...
                    mkdir($upload_path . $key, 0755);
                    chmod($upload_path . $key, 0755);
                }
