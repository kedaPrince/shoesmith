...
class File_uploader {
    ...
    public function __construct($params) {
        $name = $params['name'] or die('No name specified');
    ...
    private function validate($file = null) {
        if ($file == null) {
            ...
            if ($this->options['extensions'] && (!in_array(strtolower($file['extension']), $this->options['extensions'])))
    ...
    public static function mime_content_type($file) {
        if (function_exists('mime_content_type')) {
            ...
            if (array_key_exists($ext, $mime_types)) {
                ...
                $finfo = finfo_open(FILEINFO_MIME_TYPE);  // Should be FILEINFO_MIME_TYPE to just get the mime type
                if ($finfo) {
                    $mimetype = finfo_file($finfo, $file);
                    finfo_close($finfo);
                    return $mimetype;
                } else {
                    return 'application/octet-stream';  // Default MIME type if finfo fails
                }
            } else {
