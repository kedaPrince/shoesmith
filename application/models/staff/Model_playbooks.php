...
class Model_playbooks extends CRUD_Model
{
    ...
    {
        $loginData = $this->session->userdata['login'] ?? null;
        $this->user = $loginData ? $this->get_user_type($loginData) : null;
        $this->userAccessGroups = $this->user->user_access_groups;
    }

    private function get_user_type($loginData): ?object
    {
        if (isset($loginData['admin'])) return (object)$loginData['admin'];
        if (isset($loginData['staff'])) return (object)$loginData['staff'];
        return null;
    }

    public function selects()
    {
        edb_select('id, playbook_type_id, name, slug, generate_index_page, created_at, enabled', $this->table);
    ...
    public function wheres()
    {
        ...
        if (isset($this->userAccessGroups)) {
            $userAccessGroupIDs = array_column($this->userAccessGroups, 'id');
            $this->db->where_in('pivot_playbook_access_groups.access_group_id', $userAccessGroupIDs);
        }
        parent::wheres();
    }
    ...
    public function joins()
    {
        ...
        $this->db->join('pivot_playbook_access_groups', 'pivot_playbook_access_groups.playbook_id = ' . $this->table . '.id', 'left');
        parent::joins();
    }
