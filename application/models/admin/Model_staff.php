<?php
class Model_staff extends CRUD_Model
{
    ...
    public function get_access_groups(int $userId)
    {
        ...
        $this->db->join('pivot_staff_access_groups', 'pivot_staff_access_groups.access_group_id = mod_access_groups.id', 'inner');
        $this->db->where('pivot_staff_access_groups.staff_id', $userId);
    ...
    public function get_user_types_all()
    {
        ...
        $this->db->where_in('slug', ['general_staff', 'admin_staff']);
        $this->db->where('enabled', 1);
        $this->db->where('removed', 0);
