...
class Test_form_builder extends CRUD_Controller {
    ...
    public function quick_manage_extra($id, $row) {
        ...
        $medical_recommendations = $this->{$this->model}->get_medical_field_recommendations();
    ...
    public function create_success_extra($id)
    {
        ...
        if (!$existing_template) {
            ...
            log_message('debug', 'Template already exists for new form ID: ' . $id . ' - skipping template creation');
        }
    ...
    public function render_schema() {
        ...
        if (empty($schema_json)) {
            $this->output_json(['success' => false, 'message' => 'No schema provided']);
            return;
        }
        $schema = json_decode($schema_json, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            $this->output_json(['success' => false, 'message' => 'Invalid schema JSON']);
            return;
        }
    ...
    public function get_form_by_schema_id($schema_id = null)
    {
        ...
        if (!$schema_id || !is_numeric($schema_id)) {
            ...
            $this->output_json(['success' => false, 'message' => 'Invalid schema ID']);
            return;
        }
        $row = $this->db->where('id', $schema_id)->get('sys_form_schemas')->row();
        if (!$row) {
            log_message('error', 'Schema not found for ID: ' . $schema_id);
            $this->output_json(['success' => false, 'message' => 'Form schema not found']);
            return;
        }
        $schema = json_decode($row->schema, true);
        if (json_last_error() !== JSON_ERROR_NONE) {
            log_message('error', 'JSON decode error: ' . json_last_error_msg());
            $this->output_json(['success' => false, 'message' => 'Invalid schema format']);
            return;
        }
        try {
            $this->load->library('Form_builder');
    ...
    public function create_missing_template_sections()
    {
        ...
        foreach ($missing_sections as $form) {
            $section_data = [
                ...
                'description' => 'Auto-created template section',
                'section_type' => 'content',
                'schema_id' => $form->id,
                'enabled' => 1,
